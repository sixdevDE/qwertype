...
class ServerSync:
        ...
        for score in scores:
            try:
                response = requests.post(
                    f"{SERVER_URL}/api/scores", json=score, timeout=5
                )
    ...
    def get_leaderboard(
        self, mode: str, period: str = "alltime", limit: int = 30
    ) -> list:
...
def format_server_leaderboard(data: list, i18n, theme):
    ...
    # Header
    lines.append(
        f"<table width='100%' cellspacing='0' cellpadding='2' style='color:{base_color}'>"
    )
    lines.append(
        f"<tr>"
...
class HandRenderer:
    ...
    def set_positions_px(self, mapping: Dict[str, Tuple[int, int]]):
        ...
        self.finger_pos_px.update(
            {k: (int(v[0]), int(v[1])) for k, v in mapping.items()}
        )
...
class SquareHandPanel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        p.setPen(Qt.NoPen)
        p.setBrush(
            QColor(255, 255, 255, 6) if self.theme.is_dark else QColor(0, 0, 0, 4)
        )
...
def de_layout() -> List[List[KeyDef]]:
    return [
        ...
        [
            ...
            KeyDef("Minus", "ß", "?"),
            KeyDef("Equal", "´", "`"),
            KeyDef("Delete", "del", w=1.2),
        ],
        [
            KeyDef("Tab", "tab", w=1.3),
            KeyDef("KeyQ", "Q"),
            KeyDef("KeyW", "W"),
            KeyDef("KeyE", "E"),
            KeyDef("KeyR", "R"),
            ...
            KeyDef("KeyO", "O"),
            KeyDef("KeyP", "P"),
            KeyDef("BracketLeft", "Ü"),
            KeyDef("BracketRight", "+", "*"),
            KeyDef("Enter", "enter", w=1.6),
        ],
        [
            KeyDef("CapsLock", "caps", w=1.5),
            KeyDef("KeyA", "A"),
            KeyDef("KeyS", "S"),
            KeyDef("KeyD", "D"),
            KeyDef("KeyF", "F"),
        ...
        [
            KeyDef("ShiftLeft", "shift", w=1.7),
            KeyDef("IntlBackslash", "<", ">"),
            KeyDef("KeyY", "Y"),
            KeyDef("KeyX", "X"),
            KeyDef("KeyC", "C"),
            KeyDef("KeyV", "V"),
        ...
        [
            KeyDef("ControlLeft", "ctrl", w=1.2),
            KeyDef("AltLeft", "alt", w=1.2),
            KeyDef("Space", "space", w=6.0),
            KeyDef("AltRight", "altgr", w=1.2),
            KeyDef("ControlRight", "ctrl", w=1.2),
        ],
    ]
...
class TypingCoach:
    ...
    def suggestions(self, limit=5) -> List[str]:
        items = sorted(self.per_char_miss.items(), key=lambda x: x[1], reverse=True)[
            :limit
        ]
        out = []
...
class StartupOverlay(QWidget):
    ...
    def apply_scale(self, s: float):
        ...
        self.title.setStyleSheet(
            f"font-size: {int(22*s)}pt; font-weight: 900; color: #7c5cff;"
        )
    ...
    def paintEvent(self, event):
        ...
        p.fillRect(
            self.rect(),
            QColor(0, 0, 0, 180) if self.theme.is_dark else QColor(255, 255, 255, 140),
        )
...
class UsernameDialog(QFrame):
    ...
    def __init__(self, theme: Theme, i18n: I18N):
        ...
        self.edit.setPlaceholderText(
            self.i18n.t("settings_name_ph", fallback="nickname")
        )
    ...
    def _validate(self):
        ...
        if not is_name_allowed(name):
            self.error.setText(
                self.i18n.t(
                    "name_invalid", fallback="Invalid name (too short or blocked)."
                )
            )
        else:
...
class ScholarLessonWidget(QWidget):
    ...
    def __init__(self, parent, theme: Theme, i18n: I18N):
        ...
        self.lbl_title.setStyleSheet(
            "font-size: 20pt; font-weight: bold; color: #7c5cff;"
        )
        ...
        self.btn_back.setStyleSheet(
            f"background: {btn_bg}; border: 1px solid #232a3a; border-radius: 12px;"
        )
        self.btn_next.setStyleSheet(
            "background: #7c5cff; color: white; font-weight: bold; border-radius: 12px;"
        )
        ...
        if stype == "intro":
            ...
            if inter.get("type") == "choice":
                ...
                for opt in inter.get("options", []):
                    ...
                    btn.setStyleSheet(
                        f"background: rgba(124, 92, 255, 0.1); border: 1px solid #7c5cff; border-radius: 10px; color: #7c5cff; text-align: left; padding-left: 15px;"
                    )
            ...
            btn_play.setStyleSheet(
                "background: #ffae00; color: black; font-weight: bold; border-radius: 12px; height: 50px;"
            )
    ...
    def apply_scale(self, s):
        ...
        self.lbl_title.setStyleSheet(
            f"font-size: {int(20*s)}pt; font-weight: bold; color: #7c5cff;"
        )
...
class ResultPreviewWidget(QWidget):
    ...
    def apply_theme(self):
        ...
        self.toolbar.setStyleSheet(
            f"background: {bg}; border-bottom: 2px solid {border}; border-top-left-radius: 12px; border-top-right-radius: 12px;"
        )
        self.url_bar.setStyleSheet(f"background: {url_bg}; border-radius: 6px;")
        self.view.setStyleSheet(
            f"background: #ffffff; color: #000000; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;"
        )
        self.btn_close.setStyleSheet(
            "QPushButton { border: none; color: #6e7681; font-weight: bold; } QPushButton:hover { color: #ff5f56; }"
        )
...
class HtmlDlcWindow(QMainWindow):
    """
    Standalone OS window for HTML Scholar course.
    - Does NOT touch Main UI
    - Runs Ghost Demo locally (types into code panel + updates live preview)
    - Provides a minimal 'Now you' editor + Run/Check
    """

    def __init__(self, parent_app, theme: "Theme", i18n: "I18N", spec_data: dict):
        super().__init__(None)
        ...
        self.lbl_context.setStyleSheet(
            "font-size: 14pt; font-weight: 700; color: #7c5cff;"
        )
        hdr.addWidget(self.lbl_context)
        hdr.addStretch(1)
        ...
        self.editor.setPlaceholderText(
            "Code erscheint hier (Ghost Demo) oder du schreibst hier (Jetzt du)."
        )
        self.editor.setTabStopDistance(28)
        self.editor.textChanged.connect(self._on_editor_changed)
        rlay.addWidget(self.editor, 3)

        # Embedded preview (always visible)
        ...
        # Make it look consistent with app theme
        if getattr(theme, "is_dark", True):
            self.editor.setStyleSheet(
                "background:#0d1117; color:#e6edf3; border:2px solid #232a3a; border-radius:12px; padding:10px; font-family: Consolas, monospace;"
            )
        else:
            self.editor.setStyleSheet(
                "background:#ffffff; color:#0d1117; border:2px solid #d9dfeb; border-radius:12px; padding:10px; font-family: Consolas, monospace;"
            )

        root.addWidget(self.scholar_engine, 3)
        root.addWidget(right, 4)

        # Load initial step workspace (editor content depends on step)
    ...
    def _current_step(self) -> dict:
        if not self.scholar_engine.steps:
            return {}
        idx = getattr(self.scholar_engine, "current_idx", 0)
        if 0 <= idx < len(self.scholar_engine.steps):
            return self.scholar_engine.steps[idx]
        return {}
    ...
    def _apply_step_workspace(self):
        ...
        if stype == "ghost_demo":
            ...
            # Informational steps: show nothing or keep last
            self.editor.setReadOnly(True)
            self.lbl_context.setText("HTML Workspace")

    # ---------- ghost demo ----------
    def _on_scholar_step(self, idx: int):
        # Triggered by ▶ Run Ghost Demo button in the step UI
        if not (0 <= idx < len(self.scholar_engine.steps)):
            return
    ...
    def _start_demo_typing(self):
        try:
            self._demo_timer.timeout.disconnect(self._start_demo_typing)
        except Exception:
            pass
        if not self._demo_active:
            return
        self.ghost.total_typed = 0
    ...
    def open_html_dlc_window(self):
        ...
        if not os.path.exists(spec_path):
            self.toast.show_msg("HTML DLC Spec fehlt in data/dlc/", 2200)
            return
        try:
            with open(spec_path, "r", encoding="utf-8") as f:
                spec_data = json.load(f)
        except Exception as e:
            self.toast.show_msg(f"Spec konnte nicht geladen werden: {e}", 2400)
            return
        ...
        self._html_dlc_win = HtmlDlcWindow(self, self.theme, self.i18n, spec_data)
        self._html_dlc_win.show()


def _on_editor_changed(self):
    # Live preview while user types (only when not in ghost demo)
...
def _validate_code(self, code: str, checks: list):
    # Minimal tolerant validation (no external deps)
    low = code.lower()
    ...
    for chk in checks:
        ...
        if ctype == "element_exists":
            ...
            # Unknown check types are ignored (forward compatible)
            pass
...
def _toast(self, msg: str):
    # Use parent's toast if available, else use title flash
    try:
        if hasattr(self.parent_app, "toast"):
            self.parent_app.toast.show_msg(msg, 1800)
            return
    except Exception:
        pass
...
class DLCBrandingWidget(QFrame):
    ...
    def __init__(self, theme: Theme):
        ...
        self.lbl_main.setStyleSheet(
            "font-size: 20pt; font-weight: 900; color: #7c5cff;"
        )
...
class ScoreHeaderWidget(QFrame):
    def __init__(self, theme: Theme, i18n: I18N):
        ...
        self.lbl_score.setStyleSheet(
            "font-size: 28pt; font-weight: 900; color: #7c5cff;"
        )
...
class TrainerWidget(QFrame):
    ...
    def refresh(self, remaining: float, session_active: bool, note: str):
        ...
        if not hasattr(self, "_lint_active") or not self._lint_active:
            ...
            if sug:
                self.hints.setText(
                    self.i18n.t("suggestions") + "  " + "  |  ".join(sug)
                )
            else:
...
class SettingsWidget(QFrame):
    ...
    def __init__(self, parent_app, theme: Theme, i18n: I18N, initial_name: str):
        ...
        self.lbl_lic_title.setStyleSheet(
            "font-weight: bold; font-size: 10pt; color: #7c5cff;"
        )
...
class KeyCap(QFrame):
    def __init__(
        self, kid: str, label: str, label2: str, theme: Theme, layout_name_getter
    ):
        super().__init__()
        self.kid = kid
        self.label = label
    ...
    def paintEvent(self, event):
        ...
        if hand == "left":
            ...
            base = QColor(
                pal.get("key_bg", "#121620" if self.theme.is_dark else "#f2f5fb")
            )
        ...
        p.setFont(f)
        p.drawText(
            rect.adjusted(mx, my, -mx, -my), Qt.AlignLeft | Qt.AlignTop, self.label
        )
...
class KeyboardWidget(QFrame):
    ...
    def flash_wrong(self, kid: Optional[str]):
        ...
        kc.set_state("wrong")
        QTimer.singleShot(
            240, lambda: kc.set_state("target" if kid == self._target else "idle")
        )
        ...
        for row in self._layout_cache:
            ...
            for kd in row:
                ...
                if hand == "left":
                    ...
                    right_side.append(kd)

            for kd in left_side:
                kc = KeyCap(
                    kd.kid, kd.label, kd.label2, self.theme, lambda: self.layout_name
                )
...
class BubbleBar(QFrame):
    ...
    def paintEvent(self, e):
        ...
        p.setPen(Qt.NoPen)
        p.setBrush(
            QColor(255, 255, 255, 5) if self.theme.is_dark else QColor(0, 0, 0, 4)
        )
...
class SettingsWindow(QMainWindow):
    def __init__(self, main_window, theme: Theme, i18n: I18N, initial_name: str):
        ...
        self.settings_widget = SettingsWidget(
            self.main_window, theme, i18n, initial_name
        )
    ...
    def apply_scale(self, s: float):
        self.settings_widget._title.setStyleSheet(
            f"font-size: {max(9, int(14*s))}pt; font-weight: 800;"
        )
...
class StatsWindow(QMainWindow):
    ...
    def __init__(self, main_window, theme: Theme, i18n: I18N):
        ...
        self.mode_selector.addItems(
            ["Words", "Sentences", "Python", "JS", "C++", "Rust", "Java"]
        )
    ...
    def apply_scale(self, s: float):
        def fs(base):
            return max(6, int(base * s))

        self.score_header.lbl_score.setStyleSheet(
            f"font-size: {max(10, int(24*s))}pt; font-weight: 900; color: #7c5cff;"
        )
        self.score_header.lbl_title.setStyleSheet(
            f"font-size: {fs(10)}pt; font-weight: 600; text-transform: uppercase;"
        )
        self.score_header.lbl_name.setStyleSheet(
            f"font-size: {fs(12)}pt; font-weight: 800;"
        )
        self.score_header.lbl_meta.setStyleSheet(f"font-size: {fs(10)}pt;")
        self.leaderboard.title.setStyleSheet(
            f"font-size: {fs(15)}pt; font-weight: 800;"
        )
...
class MainWindow(QMainWindow):
    def __init__(self, theme: Theme, i18n: I18N):
        ...
        # Connect License UI
        self.settings.btn_activate.clicked.connect(
            lambda: self.license_manager.activate(self.settings.ed_key.text())
        )
        ...
        btn_stats.clicked.connect(
            lambda: self.win_stats.show() or self.win_stats.raise_()
        )
        ...
        btn_set.clicked.connect(
            lambda: self.win_settings.show() or self.win_settings.raise_()
        )
    ...
    def _refresh_leaderboard(self, mode=None):
        ...
        if hasattr(self, "server_sync"):
            if (
                hasattr(self, "lb_fetcher")
                and self.lb_fetcher
                and self.lb_fetcher.isRunning()
            ):
            ...
            self.lb_fetcher = LeaderboardFetcher(
                self.server_sync, target_mode, username=self.name, limit=30
            )
    ...
    def _fetch_user_bests(self, username: str):
        ...
        # Fetch data
        self.lb_fetcher = LeaderboardFetcher(
            self.server_sync, self.mode, username=username
        )
    ...
    def _apply_responsive_sizes(self):
        ...
        # Container layouts
        self.workspace_lay.setContentsMargins(
            int(20 * s), int(6 * s), int(20 * s), int(6 * s)
        )
        self.workspace_lay.setSpacing(int(8 * s))

        self.keyboard_lay.setContentsMargins(
            int(12 * s), int(6 * s), int(12 * s), int(6 * s)
        )
        ...
        self.trainer.word_label.setStyleSheet(
            f"font-size: {max(12, int(24*s))}pt; font-weight: 900;"
        )
        self.trainer.stats.setStyleSheet(f"font-size: {fs(11)}pt;")
        self.trainer.hints.setStyleSheet(
            f"font-size: {fs(11)}pt; color: #7c5cff; font-weight: bold;"
        )
    ...
    def _sync_dlc_ui(self):
        ...
        try:
            if self.mode == "html_scholar" and getattr(self, "dlc_manager", None):
                # Run HTML course in a separate OS window (standalone)
                self.open_html_dlc_window()

                # Important: keep Main UI untouched and exit scholar mode immediately
                self.mode = "words"
                try:
        ...
        if dlc and self.mode not in [
            ...
            self.dlc_header.set_content(
                dlc.get("branding", {}).get("text", "DLC MODULE")
            )
            ...
            self.overlay.title.setText(
                self.i18n.t("hint_type_start_title", fallback="READY?")
            )
    ...
    def _run_demo(self, demo_data: dict):
        ...
        self.trainer.refresh(
            60.0, False, f"DEMO: {demo_data.get('name', 'Watching...')}"
        )
    ...
    def _stop_demo(self):
        ...
        if hasattr(self, "ghost"):
            self.ghost.stop()
            try:
                self.ghost.char_typed.disconnect()
            except:
                pass
            try:
                self.ghost.step_finished.disconnect()
            except:
                pass
        self._demo_steps = None
        self._current_demo_explain = ""
    ...
    def _next_demo_step(self):
        try:
            ...
            self._current_demo_explain = (
                f"📘 {self._current_step_data.get('explain', 'Teaching...')}"
            )
            ...
            # Show the fake browser preview after a short delay
            full_code = self.coach.current
            QTimer.singleShot(1500, lambda: self.result_viewer.show_content(full_code))

            # Show the scholar engine or overlay back after viewing result
            QTimer.singleShot(
                6000, lambda: self.show_startup_overlay() if self._demo_active else None
            )
    ...
    def end_session(self):
        ...
        self.score_header.set_score(
            self.last_points, self.last_wpm, self.last_acc, self.name
        )
        ...
        if self.last_points > 0:
            self.server_sync.add_score(
                ...
                completion_pct=(
                    100.0
                    if not self.coach.items
                    else (self.coach.index / len(self.coach.items)) * 100
                ),
            )
    ...
    def tick(self):
        ...
        self.trainer.refresh(
            remaining=remaining, session_active=self.session_active, note=note
        )
    ...
    def eventFilter(self, obj, e):
        if e.type() == QEvent.KeyPress and e.text():
            ...
            if (
                hasattr(self, "ghost")
                and self.ghost.timer.isActive()
                or self._demo_timer.isActive()
            ):
            ...
            expected_kid = (
                kid_for_char(self.layout, expected.lower()) if expected else None
            )
